...
def featurecalculation_di(seqscore,separation):
    scorez = []
    eucdistance = []
    for i in range(len(seq)):
        eucdistance.append([])
...
if __name__=="__main__":
    ...
    seqscore=di_abs('F:\\sab_new_bound_features\\2.0\\2.0_latest\\new_test\\di\\','_202107312216_unbound.tbl')
    ...
    for each_sep in range(len(radii)):
        di_values['2.0_di_avg_bound_'  + radii_names[each_sep]] = featurecalculation_di(seqscore, separation=radii[each_sep])
    df = pd.DataFrame(di_values)
    df.to_csv('F:\\sab_new_bound_features\\2.0\\2.0_latest\\new_test\\features\\new_di_test_avg.csv', index=False)
