...
def featurecalculation_cn(separation):
    scorez = []
    eucdistance=[]
    for i in range(len(seq)):
        eucdistance.append([])
    ...
    for i in range(len(seq)):
        for j in range(len(lengthofcontacts[i])):
            final_feature.append(lengthofcontacts[i][j])
...
if __name__=="__main__":
    ...
    for each_sep in range(len(radii)):
        contact_num['2.0_CN_'  + radii_names[each_sep]+'_abs_bound'] = featurecalculation_cn(separation=radii[each_sep])
    df = pd.DataFrame(contact_num)
    df.to_csv('F:\\sab_new_bound_features\\2.0\\2.0_latest\\new_test\\features\\new_cn_test_abs.csv', index=False)
